{{-- script ของหน้าระดับตำแหน่งงาน ดูแล modal, validation, submit และ auto-hide message --}}
<script>
    let isFormValid = false;
    const jobLevelUpdateAction = "{{ route('job-level.update', ':id') }}";
    ...
    function validateForm() {
        ...
        if (nameValue === '') {
            ...
            nameError.textContent = 'กรุณากรอกชื่อระดับตำแหน่งงาน';
            isValid = false;
        } else {
    ...
    // เปิด modal สำหรับเพิ่มระดับตำแหน่งงานใหม่ และรีเซ็ตฟอร์มกลับเป็นสถานะเริ่มต้น
    function openCreateModal() {
        ...
        modalTitle.innerHTML = '<i class="fas fa-plus me-2"></i>เพิ่มระดับตำแหน่งงาน';
    ...
    // ดึงข้อมูลเดิมจากปุ่มในตารางมาเติมใน modal เพื่อแก้ไขระดับตำแหน่งงานเดิม
    function handleEdit(id, name) {
        ...
        resetForm();
        form.action = jobLevelUpdateAction
            .replace(':id', encodeURIComponent(id))
            .replace('%3Aid', encodeURIComponent(id));
        document.getElementById('form_method').value = 'PUT';
        document.getElementById('positionId').value = id;
        document.getElementById('name').value = name || '';
        modalTitle.innerHTML = '<i class="fas fa-edit me-2"></i>แก้ไขระดับตำแหน่งงาน';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('click', function(event) {
            const button = event.target.closest('[data-job-level-edit]');
            if (button) handleEdit(button.dataset.jobLevelId, button.dataset.jobLevelName || '');
        });
