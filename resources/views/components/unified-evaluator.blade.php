                        ...
                        <div class="{{ $hasQualityItems ? 'relative pr-14' : '' }} bg-gradient-to-r from-purple-100 to-blue-100 px-6 py-4 rounded-t-lg border-b border-gray-200">
                            ...
                            @if(!empty($evaluationList['annotation']))
                                <div class="flex items-center space-x-2 mt-1">
                                    <div class="text-sm text-gray-600">{!! \App\Support\SafeHtml::richText($evaluationList['annotation']) !!}</div>
                                </div>
                            @endif
                            @if($hasQualityItems)
                                        ...
                                        <div class="space-y-3 ml-6 mb-6">
                                            @if(!$readonly)
                                                <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                                                    <div class="font-semibold mb-1">คำชี้แจง</div>
                                                    <div>กรุณาตรวจสอบรายการย่อยและกรอกคะแนนให้ครบถ้วนก่อนบันทึก</div>
                                                    <div>ระบบจะคำนวณคะแนนจากข้อมูลที่กรอกโดยอัตโนมัติ</div>
                                                    <div>หากข้อมูลไม่ครบ ระบบอาจคำนวณคะแนนได้ไม่สมบูรณ์</div>
                                                </div>
                                            @endif
                                                        ...
                                                        <div class="rounded-xl border border-slate-200 bg-white/90 p-4 shadow-sm">
                                                            <div class="space-y-4">
                                                                @if(!empty($subCriteria['description']))
                                                                    <div class="rounded-lg bg-slate-50 px-3 py-2 text-sm leading-6 text-slate-600">{!! \App\Support\SafeHtml::richText($subCriteria['description']) !!}</div>
                                                                @endif
                                                                ...
                                                                {{-- Score B --}}
                                                                <div class="flex h-full flex-col rounded-xl border border-slate-200 bg-slate-50/80 p-3">
                                                                    <label for="evaluator-score-b-{{ $subCriteria['id'] }}" class="mb-2 flex min-h-[32px] items-center justify-center text-center text-sm font-semibold leading-6 text-slate-700">
                                                                        หน่วยภาระงานมาตรฐาน (B)
                                                                    </label>
                                                                    <input type="text"
                                                                        id="evaluator-score-b-{{ $subCriteria['id'] }}"
                                                                ...
                                                                <div class="flex h-full flex-col rounded-xl border border-blue-200 bg-blue-50/60 p-3">
                                                                    <label for="evaluator-score-c-{{ $subCriteria['id'] }}" class="mb-2 flex min-h-[32px] items-center justify-center text-center text-sm font-semibold leading-6 text-slate-700">
                                                                        หน่วยภาระงานที่ทำได้ (C)
                                                                    </label>
                                                                    @if(!$readonly)
                                                                        <input type="number" step="1" min="0"
                                                                            id="evaluator-score-c-{{ $subCriteria['id'] }}"
                                                            ...
                                                            @if(!$readonly || !empty($subCriteria['score_histories']))
                                                                <div class="rounded-xl border border-amber-200 bg-amber-50/70 p-3">
                                                                    <label for="evaluator-score-description-{{ $subCriteria['id'] }}" class="@if($readonly) hidden @else mb-2 block text-sm font-semibold leading-6 text-slate-700 @endif">
                                                                        หมายเหตุการแก้ไขค่า C
                                                                    </label>
                                                                    @if(!$readonly)
                                                                        <textarea
                                                                            id="evaluator-score-description-{{ $subCriteria['id'] }}"
                                                                        ...
                                                                        <div class="mt-3 hidden" data-score-change-reason-container>
                                                                            <label for="evaluator-quantity-reason-{{ $subCriteria['id'] }}" class="mb-2 block text-sm font-semibold text-slate-700">
                                                                                เหตุผลที่แก้ไขคะแนน
                                                                            </label>
                                                                            <textarea
                                                                                id="evaluator-quantity-reason-{{ $subCriteria['id'] }}"
                                                                    ...
                                                                    @else
                                                                        <textarea
                                                                            id="evaluator-score-description-readonly-{{ $subCriteria['id'] }}"
                                                                            rows="3"
                                                                            readonly
                                    ...
                                    <div class="mt-6">
                                        @foreach($evaluationList['quality_items'] as $mainCriteria)
                                            ...
                                            <details class="group border border-gray-300 rounded-lg bg-white mb-6 main-criteria"
                                                ...
                                                <div class="px-4 py-4">
                                                    @if(!empty($mainCriteria['tooltips']))
                                                        <div class="text-sm text-gray-500 mb-3">{!! \App\Support\SafeHtml::richText($mainCriteria['tooltips']) !!}</div>
                                                    @endif
                                                    ...
                                                    <div class="space-y-3 ml-2">
                                                        ...
                                                        class="h-5 w-5 text-purple-600 focus:ring-purple-500 border-gray-300 rounded mr-3">
                                                                        ...
                                                                        @else
                                                                            <input type="checkbox"
                                                                                id="evaluator-quality-criteria-readonly-{{ $subCriteria['id'] }}"
                                                                                {{ $shouldBeChecked ? 'checked' : '' }}
                                                                                disabled
                                                                    ...
                                                                    @if(!$readonly)
                                                                        ...
                                                                        <input type="hidden"
                                                                            ...
                                                                            data-list-max="{{ $evaluationList['sum_score'] ?? 0 }}">
                                                                        <div class="mt-3 hidden" data-score-change-reason-container>
                                                                            <label for="evaluator-quality-reason-{{ $subCriteria['id'] }}" class="mb-2 block text-sm font-semibold text-slate-700">เหตุผลที่แก้ไขคะแนน</label>
                                                                            <textarea
                                                                                id="evaluator-quality-reason-{{ $subCriteria['id'] }}"
                                                        ...
                                                        @php
                                                            $links =
                                                                ...
                                                                is_array($qualityEvidenceMap[$mainCriteria['id']])
                                                                    ? $qualityEvidenceMap[$mainCriteria['id']] : (isset($qualityEvidenceMap[$mainCriteria['id']]) ? [$qualityEvidenceMap[$mainCriteria['id']]] : ['']);
